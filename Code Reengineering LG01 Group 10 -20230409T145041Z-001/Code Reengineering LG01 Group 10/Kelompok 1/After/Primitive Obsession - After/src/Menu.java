...
public class Menu {
	...
	private void saveChar() {
		...
		armorName = sc.nextLine();		
		armorType = insertArmorType();
		clearScreen();
		
		insertChar(name, charClass, weaponName, weaponType, armorName, armorType);
	...
	private void viewChar() {
		if(cl.isEmpty()) {
			...
			for(int i = 0; i < count; i++) {
				System.out.printf("||  %-3d | %-13s | %-14s | %-21s | %-13s | %-21s | %-13s ||\n", i + 1, cl.getName(i), cl.getCharClass(i), cl.getWeapon(i).getWeaponName(), cl.getWeapon(i).getWeaponType(), cl.getArmor(i).getArmorName(), cl.getArmor(i).getArmorType());
			}
		}
	}
